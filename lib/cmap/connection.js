...
class Connection extends mongo_types_1.TypedEventEmitter {
        ...
        try {
            ...
            for await (document of this.sendWire(message, options, responseType)) {
                ...
                if (document.ok === 0) {
                    ...
                    // Enhanced error handling for pipeline errors
                    const errorObject = object ??= document.toObject(bsonOptions);
                    const mongoError = new error_1.MongoServerError(errorObject);
                    // Check for the specific pipeline error code 40323
                    if (mongoError.code === 40323) {
                        const commandInfo = {
                            command: command,
                            database: ns.db,
                            collection: ns.collection,
                            pipeline: command.pipeline || 'No pipeline found'
                        };
                        const enhancedMessage = `Pipeline Error (40323): ${mongoError.message}\n\nCommand Details:\nDatabase: ${commandInfo.database}\nCollection: ${commandInfo.collection}\nPipeline: ${JSON.stringify(commandInfo.pipeline, null, 2)}\n\nFull Command: ${JSON.stringify(commandInfo.command, null, 2)}`;
                        // Use the existing logger if available, otherwise use console
                        if (this.mongoLogger) {
                            this.mongoLogger.error(mongo_logger_1.MongoLoggableComponent.COMMAND, {
                                toLog: () => ({
                                    message: 'CRITICAL PIPELINE ERROR DETECTED',
                                    errorDetails: enhancedMessage,
                                    command: commandInfo
                                })
                            });
                        }
                            ...
                            console.error('CRITICAL PIPELINE ERROR DETECTED:');
                            // eslint-disable-next-line no-console
                            console.error(enhancedMessage);
                            // eslint-disable-next-line no-console
                            console.error('\nExiting application due to pipeline error...');
                        }
    ...
    async command(ns, command, options = {}, responseType) {
        ...
        for await (const document of this.sendCommand(ns, command, options, responseType)) {
            ...
            // Additional check for pipeline errors in the main command method
            if (document.ok === 0 && document.code === 40323) {
                const commandInfo = {
                    command: command,
                    database: ns.db,
                    collection: ns.collection,
                ...
                // Handle both Document and MongoDBResponse types
                const errorMessage = 'errmsg' in document ? document.errmsg :
                    'message' in document ? document.message :
                        'Unknown error';
                const enhancedMessage = `Pipeline Error (40323): ${errorMessage}\n\nCommand Details:\nDatabase: ${commandInfo.database}\nCollection: ${commandInfo.collection}\nPipeline: ${JSON.stringify(commandInfo.pipeline, null, 2)}\n\nFull Command: ${JSON.stringify(commandInfo.command, null, 2)}`;
                // Use the existing logger if available, otherwise use console
                if (this.mongoLogger) {
                    this.mongoLogger.error(mongo_logger_1.MongoLoggableComponent.COMMAND, {
                        toLog: () => ({
